...
import { uploadAttachment } from '../../lib/photo';
...
/** The proof photo is filed by the shared photo module — one
 * implementation of the checksum + multipart dance, shared with the
 * technician's completion photos (2026-09-16). */
async function uploadProofPhoto(paymentId: string, fileUri: string): Promise<void> {
  await uploadAttachment({ ownerType: 'payment', ownerId: paymentId, kind: 'photo', fileUri, fileName: 'proof.jpg' });
}
